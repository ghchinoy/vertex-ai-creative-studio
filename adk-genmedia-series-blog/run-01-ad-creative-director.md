---
title: "The Creative Director's Assistant: a brand brief becomes a finished, on-brand ad â automatically"
...
merged: "PR #1821 (squash-merge a8ce2df on main) â feat(adk-series): Ad creative-director capstone (SequentialAgent â ParallelAgent + AgentTool)"
...
# The Creative Director's Assistant: a brand brief becomes a finished, on-brand ad â automatically

> _[Hero illustration rendering — the final art is being produced on the credentialed render channel; art-direction: `illustrations/run-01-ad-creative-director-hero.md`.]_
...
And here is what comes back: **one finished short video ad** â `final_ad.mp4` â with a planned set
...
> This is the payoff of the whole series. Everything you built in the earlier steps â the
> photographer, the videographer, the music producer, the way collaborators hand work down the line
> â now works together as **one studio**, run by a creative director you brief in a sentence.
...
1. **Plan.** A creative director reads the brief and writes a tight, duration-budgeted shot plan â
...
2. **Shoot â all at once.** Each planned shot goes to its own shot team, and the teams work **in
   parallel**. Every team calls the **Photoshoot** for a still, then the **Director / Videographer**
   to animate that still into a clip â the exact personas you already built.
3. **Score.** The **Music Producer** â again, the one you already built â lays down a music bed in
...
4. **Assemble.** An editor stitches the clips in order, mixes the music under the voice, trims the
   audio to the picture, and lays it over the video â then confirms the finished file is really there.
...
## Reuse, don't rebuild â the whole point of the capstone
...
```python
# The specialists you already shipped, reused verbatim â not rewritten.
...
the Photoshoot, it runs the *real* Photoshoot â its own instructions, its own image tool, its own
"verify the file exists" habit â in its own private workspace, so several shot teams can call the
same specialists at the same time without stepping on each other.

*Powerful, made approachable:* every craft lesson from the earlier posts â the explicit Veo-3 model
that keeps video sound working, the music producer's naming crosswalk, verify-by-existence â is
...
checkout. It's one example reusing the examples beside it â and if one is missing, it says so
...
![How it works: your brief â the planner writes a schema-checked AdPlan â three shot teams run in parallel (each: Photoshoot still â Director clip) â the Music Producer scores it â the assembler cuts, mixes, trims, and verifies one final ad](diagrams/ad-creative-director.png)
...
The creative director's only job is to think â it has **no tools at all**. It reads your brief and
...
has a guaranteed shape, every downstream stage can rely on it â the shot teams know exactly where to
...
model actually supports. A plan that breaks those rules is rejected the moment it's written â so a
...
a limitation you'll trip over â it's the honest math of a short ad. Each shot becomes a video clip,
and the video model produces clips of 4, 6, or 8 seconds. Three shots therefore give you 12â24
seconds of distinct hero footage â exactly the shape of a bumper or short-form ad, comfortably
...
both sides â in the form's rules and in the director's instructions â so nothing is ever silently
dropped.)*
...
editor trims the mixed audio down to the exact length of the cut picture before the final combine â
...
doesn't expose the one setting you need. The honest move â the one this build models â is a small,
...
This is the one agent that exercises **every** genmedia server â stills, video, music, voice, and
the final assembly â so make sure the media toolkit (the genmedia MCP suite, plus `ffmpeg`) is
...
a music bed and a voiceover, and assembles `./output/final_ad.mp4` â reporting the verified path and
the measured duration.

*(You may see a little teardown noise in the logs as the parallel shot teams finish â the kind of
...
editor confirms `final_ad.mp4` by **actually finding the file and measuring it** â reading its
media info and listing it on disk â never by trusting a "here's your link" response. It matters most
here because the video step returns exactly such a link, and a link is not proof that anything was
saved. If the final file isn't really there, you're told â you never get a green checkmark over an
...
- **`countdown-workflow`** â a hand-written pipeline that does this same end-to-end job (script â
  first-frame stills â continuous clips â validate â compose with music) on a different surface. This
...
- **The `story-generator` skill** â its "writers' room â generate every scene" shape, and its
...
You've reached the finale â a brand brief becomes a finished, on-brand ad, built entirely from the
...
shipped tree at `ad-creative-director/`). Code is condensed for reading; the full wiring â the
`sys.path` sibling imports, the planner base instruction, the fixed three-slot shot stage, the
assembler's avtool tool-filter and the `trim_audio_to_video_length` helper â is in
