...
def settings_page_content(app_state: me.state):
    ...
    with page_scaffold():  # pylint: disable=not-context-manager
        with page_frame():  # pylint: disable=not-context-manager
            ...
            me.text(
                f"Vote pause time: {Default.SHOW_RESULTS_PAUSE_TIME} seconds",
            )
    ...
    if len(studies):
        ...
        for study in studies:
            with me.box(style=_BOX_STYLE):
                ...
                me.text(
                    f"{study_modifier} Study: {this_study['label']}",
        ...
        me.button(
            label="Reset Leaderboard for current study",
            on_click=lambda study=app_state.study: _handle_purge(study),
            key=f"{app_state.study}",
        )
