...
class Default:
    ...
    MODEL_STABLE_DIFFUSION_ENDPOINT_ID: str = os.environ.get(
        "MODEL_STABLE_DIFFUSION_ENDPOINT_ID",
    )
    ...
    def __post_init__(self):
        ...
        if not self.MODEL_FLUX1_ENDPOINT_ID:
            print(
                "MODEL_FLUX1_ENDPOINT_ID environment variable is not set. List of models will exclude flux1",
            )  # Optional: List of models will exclude flux1

        if not self.MODEL_STABLE_DIFFUSION_ENDPOINT_ID:
            print(
                "MODEL_STABLE_DIFFUSION_ENDPOINT_ID environment variable is not set. List of models will exclude stable diffusion",
            )
        ...
        if not self.IMAGE_FIREBASE_DB:
            raise ValueError(
                "IMAGE_FIREBASE_DB environment variable is not set. Default will be used",
            )
