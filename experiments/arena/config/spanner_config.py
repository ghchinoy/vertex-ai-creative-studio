...
class ArenaModelEvaluation:
    ...
    def __post_init__(self):
        ...
        log(
            f"Initialized StudyRun: {self.model_name}, {self.time_of_rating}, {self.rating}, {self.study}, {self.id}",
        )
...
class ArenaStudyTracker:
    ...
    def upsert_study_runs(
        self,
        study_runs: list[ArenaModelEvaluation],
        table_name: str | None = "Study",
    ):
        ...
        for study_run in study_runs:
            ...
            if not study_run.time_of_rating:
                ...
                is_insert = True
                log(
                    "Setting time_of_rating to commit timestamp as it was not provided.",
                )
...
class ArenaStudySchema:
    ...
    def create_database(self, exists_ok: bool = False):
        ...
        try:
            ...
            log(
                f"Adding to Spanner instance {self.spanner_instance_id} in project {self.project_id}.",
            )
    ...
    def drop_schema(self):
        ...
        try:
            log("Dropping schema.")
            operation = self.database.update_ddl(
                ["DROP INDEX StudyByModel", "DROP TABLE Study"],
            )
            operation.result(config.SPANNER_TIMEOUT)
