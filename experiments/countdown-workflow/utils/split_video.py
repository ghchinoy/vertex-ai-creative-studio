...
def split_video_into_chunks(
    ...
    try:
        with VideoFileClip(input_filepath) as clip:
            ...
            logger.info(
                f"Splitting '{input_filepath}' (Duration: {duration:.2f}s) into {num_chunks_to_process} chunks (skipping the last one) of {chunk_duration_seconds}s...",
            )
            ...
            for i in range(num_chunks_to_process):
                ...
                logger.info(
                    f"Processing chunk {i + 1}/{num_chunks_to_process}: {output_filename} (from {start_time:.2f}s to {end_time:.2f}s)",
                )
                try:
        ...
        logger.error(
            f"Error loading video with MoviePy. Ensure FFmpeg is installed and the video file is valid: {e}",
        )
        return
