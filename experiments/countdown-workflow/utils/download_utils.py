...
_downloaded_file_path_global: str | None = None
...
def download_ranges_callback(info_dict: dict, ydl: YoutubeDL) -> list[dict]:
    ...
    if not start_time or not end_time:
        raise ValueError(
            "start_time and end_time must be provided in ydl.params for download_ranges_callback.",
        )
...
def my_progress_hook(d: dict) -> None:
    ...
    if d["status"] == "finished":
        ...
        logger.info(
            f"Download finished. Final filename captured by hook: {_downloaded_file_path_global}",
        )
