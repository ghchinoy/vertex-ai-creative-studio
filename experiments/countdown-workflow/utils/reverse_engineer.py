...
class PromptAnalysis(BaseModel):
    ...
    scene_number: int = Field(
        description="The sequential number of the video chunk, starting from 1.",
    )
    image_prompt: str | None = Field(
        default=None,
        description="The inferred Imagen prompt for the first frame of the entire video. This should only be present for the first scene.",
    )
    video_prompt: str = Field(
        description="The inferred Veo prompt for this specific video chunk, including action, camera movement, and art style.",
    )
...
def reverse_engineer_prompts(
    ...
    logger.info(
        f"Starting prompt reverse engineering for videos in '{chunks_dir}' using model '{config.REVERSE_ENGINEERING_MODEL}' via Vertex AI...",
    )
    ...
    try:
        ...
        for i, video_filename in enumerate(video_files):
            ...
            if extract_first_frame(video_filepath, frame_image_path):
                ...
                logger.warning(
                    f"  - Could not extract the first frame for chunk {video_filename}.",
                )
...
if __name__ == "__main__":
    ...
    _full_video_filepath = (
        "video/Google I⧸O '25 Keynote.mp4"  # Replace with a valid path
    )
    ...
    if os.path.exists(_full_video_filepath) and os.path.exists(_chunks_dir):
        ...
        logger.warning(
            f"Cannot run standalone example: Video file '{_full_video_filepath}' or chunks directory '{_chunks_dir}' not found.",
        )
